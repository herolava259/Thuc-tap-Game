...
void OptWinObject::SetBackGround(const std::string& str) {
	...
	m_bgWindow = ResourceManagers::GetInstance()->GetTexture(str);
}
...
void OptWinObject::TurnOn() {
	m_OnWindow = true;
	m_currentFrame = 0;
	m_currentTime = 0.0f;
	m_pTexture = m_bgAnimation;
}
void OptWinObject::TurnOff() {
	m_OnWindow = false;
	
}
void OptWinObject::Draw_All() {
	if (!m_endAnimation) {
		this->Draw_Animation();
	}
	else {
...
void OptWinObject::HandleTouchEvents(int x, int y, bool bIsPressed) {
	for (auto it : m_listButton) {
		it.second->HandleTouchEvents(x, y, bIsPressed);
		if (it.second->IsHandle()) break;
	}
}